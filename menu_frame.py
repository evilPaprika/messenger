...
class Menu(Frame):
    def __init__(self, parent):
        ...
        Label(self, text="enter ip address:").grid(row=1, column=0, sticky="E")
    ...
    def connect_action(self):
        address = (self.connect_entry.get(), 25000)
        self.parent.update_tab(chat_frame.Chat(self.notebook, "connect", address), self.connect_entry.get())

    def host_action(self):
        address = ("", 25000)
        self.parent.update_tab(chat_frame.Chat(self.notebook, "host", address), '{}:{}'.format(address[0], address[1]))
